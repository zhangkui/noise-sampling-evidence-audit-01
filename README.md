...
- **批次详情与汇总**：`GET /api/batches/{id}` 返回批次基本信息（不存在返回 **40400**，`CLOSED` 同样可查）；
  `GET /api/batches/{id}/statistics` 在数据库端按 `batch_id` 精确聚合记录总数、首末采样时间、
  平均/最大/最小分贝、异常总数及各状态数量，支持大批量记录（走 `idx_record_batch` 索引，不把记录载入内存）。
  **空批次约定**：记录数与异常计数为 0，首末采样时间与分贝统计为 `null`（前端展示“—”）。
  统计为纯只读操作，不修改记录、异常、证据，也不追加任何审计节点。
...
| 传感器与批次 | 传感器维护、批次维护、**时间重叠冲突检测**、**批次关闭（确认/加载态/失败重试）**、行/「详情/统计」进入批次详情 |
| 批次详情 | 基本信息、**统计卡片**（记录数/平均/最大/最小分贝/首末采样时间）、**异常状态分布图 + 明细表**、空批次/404/失败重试、带 `batchId`（可叠加异常状态）跳转采样记录 |
...
| GET | /api/batches | 批次列表（可按传感器过滤） |
| GET | /api/batches/{id} | 批次详情（不存在 40400，CLOSED 可查） |
| GET | /api/batches/{id}/statistics | **批次汇总统计（只读，空批次分贝字段为 null）** |
